"""Admin router: manual rollup refresh (SP-only)."""
...
async def refresh_rollup(days: int = Query(7, ge=1, le=90)) -> dict:
    """Recompute geniewatch.daily_usage_rollup over the last N days.

    Synchronous (no background queue). Pulls per-space queries + cost from
    system tables, joins with feedback counts, and upserts one row per
    (space_id, day).
    """
    ...
    for u in usage:
        ...
        c = cost.get(sid) or {}
        f = fb.get(sid) or {}
        # We collapse to a single row at "today" since the SP queries above are
        # already day-aggregated rollups; the per-day breakdown is available
        # via /api/spaces/{id}/cost and /api/spaces/{id}/usage.
        await lakebase.upsert_daily_rollup({
            "space_id": sid,
            "day": today,
