...
Forked from genie-workbench. GenieWatch is read-only — `query_genie_for_sql`
...
def get_genie_space(genie_space_id: str) -> dict:
    """Fetch a Genie space's serialized configuration. Tries OBO, falls back to SP."""
...
def list_genie_spaces() -> list[dict]:
    """Paginate through all Genie Spaces visible to the current identity."""
    client = get_workspace_client()
    try:
...
def list_space_permissions(genie_space_id: str) -> dict:
    """Fetch the ACL for a Genie space.

    Genie spaces use Databricks' generic permissions API at
    /api/2.0/permissions/genie/{id}. Returns the raw response, which the
    caller can map onto a Pydantic model.
    """
    if not genie_space_id:
        raise ValueError("genie_space_id is required")
    client = get_workspace_client()
