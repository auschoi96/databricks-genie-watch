...
The Genie REST API does not expose a workspace-wide conversation list — you
must enumerate per space. This module paginates `/api/2.0/genie/spaces/{id}/
conversations` (and optionally `/messages` for each conversation) and writes
the result into Lakebase via `services.lakebase`.

Always runs as the service principal so a single sync covers every space
regardless of which user is browsing the app.
"""
...
def _list_conversations(space_id: str) -> list[dict[str, Any]]:
    """Paginate /api/2.0/genie/spaces/{id}/conversations."""
    client = _client()
...
async def sync_space(space_id: str, fetch_messages: bool = True) -> dict[str, int]:
    """Sync conversations (and optionally messages) for one space.

    Returns a small summary {conversations, messages} for the caller.
    """
    ...
    try:
        ...
        await lakebase.set_watermark(resource_key, status="error", error=str(e))
    ...
    await lakebase.set_watermark(resource_key, status="ok")
...
def _ts(value) -> datetime | None:
    ...
    if isinstance(value, datetime):
        return value
    s = str(value)
    # Accept either ISO-8601 or epoch milliseconds
    try:
        if s.isdigit():
...
def _extract_sql(message: dict) -> str | None:
    """Pull SQL out of a message attachment if present."""
